    ...
    <div class="container mt-5">
      <div *ngFor="let post of posts">
        <div class="mb-3" *ngIf="!post.active" [ngClass]="changeCol(post)">
          <h5 class="card-header"><span highlight>Inactive post</span></h5>
          <div class="card-body">
            <h5 clas>{{ post.id }} - {{ post.title | uppercase }}</h5>
            <p class="card-text">{{ post.body }}</p>
            <button class="btn btn-secondary form-control mb-3" (click)="changeStatus(post)">Attiva</button>
            <button class="btn btn-secondary form-control" [routerLink]="['/post/inactive', post.id]">Dettagli</button>
          </div>
        </div>
      </div>
    </div>
